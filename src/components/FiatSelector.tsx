  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <motion.button
        ...
        className="flex items-center gap-2 px-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 hover:border-slate-600 transition-all"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full mt-2 right-0 w-64 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50"
          >
            <div className="p-2">
              {FIAT_CURRENCIES.map((currency) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-white">{currency.code}</div>
                    <div className="text-xs text-slate-400">{currency.name}</div>
                  </div>
                  <div className="text-lg text-slate-300">{currency.symbol}</div>
                </button>
              ))}
            </div>
          </motion.div>
